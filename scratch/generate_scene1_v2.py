...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Deep room wall with subtle acoustic slats and directional studio light */
  .wall {
    position: absolute;
    top: 0; left: 0; width: 1080px; height: 1180px;
    background: radial-gradient(circle at 70% 30%, #162030 0%, #0c1119 55%, #070a0f 100%);
  }
  ...
  .acoustic-slats {
    ...
    mask-image: radial-gradient(circle at 60% 35%, rgba(0,0,0,0.8) 0%, transparent 80%);
  }

  /* Monitor backlight glow on wall */
  .monitor-ambient-glow {
    position: absolute;
    top: 100px; left: 50%;
    transform: translateX(-50%);
    width: 1040px; height: 860px;
    background: radial-gradient(ellipse at center, rgba(56, 189, 248, 0.16) 0%, rgba(30, 58, 138, 0.08) 50%, transparent 75%);
    filter: blur(60px);
  }

  /* Warm desk lamp glow from top right */
  .lamp-glow {
    position: absolute;
    top: 0px; right: 0px;
    width: 550px; height: 1400px;
    background: radial-gradient(ellipse at 90% 10%, rgba(245, 158, 11, 0.09) 0%, rgba(245, 158, 11, 0.03) 45%, transparent 75%);
    filter: blur(50px);
    pointer-events: none;
  }

  /* Upper Studio Shelf / Bokeh lights */
  .studio-shelf {
    position: absolute;
    top: 50px; left: 60px; width: 960px; height: 4px;
  ...
  .nav-title {
    height: 10px; width: 65px; background: #334155; border-radius: 4px; margin-bottom: 6px;
  }
  .tree-row {
  ...
  .inactive-tab {
    height: 28px; width: 115px;
    display: flex; align-items: center; padding: 0 10px; gap: 8px; opacity: 0.35;
  }
  ...
  .c-line-num {
    width: 22px; height: 7px; background: #1e293b; border-radius: 3px; margin-right: 12px;
  }
  ...
  /* Bottom Integrated Terminal in IDE */
  .ide-term {
    height: 140px;
    background: #070a0f;
  ...
  .spark-chart {
    height: 40px;
    background: linear-gradient(180deg, rgba(56, 189, 248, 0.15) 0%, rgba(56, 189, 248, 0) 100%);
  ...
  /* DESK SURFACE - Stretches from 1040px all the way to 1920px (880px tall) */
  .desk-plane {
    position: absolute;
    top: 1040px;
    left: 0;
    ...
    box-shadow: 
      inset 0 30px 60px rgba(56, 189, 248, 0.04),
  ...
  .mouse {
    width: 90px;
    height: 155px;
    background: linear-gradient(180deg, #1d2638 0%, #111722 100%);
    border-radius: 45px;
    box-shadow: 
  ...
  /* Safe Area subtle tone - clean for subtitles */
  .safe-area-lower {
    position: absolute;
    bottom: 0; left: 0; width: 1080px; height: 420px;
    background: linear-gradient(180deg, transparent 0%, rgba(7, 10, 15, 0.6) 100%);
    pointer-events: none;
  }
</style>
</head>
<body>
  ...
  <!-- Full Lower Desk Assembly (Extends to bottom edge 1920px) -->
...
Path("scratch/scene1_v2.html").write_text(html, encoding="utf-8")
subprocess.run([
    "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome",
    "--headless",
    "--screenshot=scratch/scene1_v2.png",
    "--window-size=1080,1920",
    "--hide-scrollbars",
    f"file://{Path('scratch/scene1_v2.html').resolve()}"
], check=True)
print("Rendered Scene 1 v2!")
